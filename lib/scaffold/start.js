...
function checkConfigVersion2(fullConfig) {
    ...
    if (!datadirUndefined || addressDefined || dbDefined) {
        ...
        console.warn('\nConfiguration file is not compatible with this version. \n' +
            ...
            'There are changes necessary in both bitcoin.conf and bitcore-node-jemcash.json. \n\n' +
            'To upgrade please see the details below and documentation at: \n' +
            'https://github.com/jemcashproject/bitcore-node-jemcash/blob/jemcashd/docs/upgrade.md \n');
                ...
                servicesConfig: {
                    bitcoind: {
                        spawn: {
                            ...
                            exec: path.resolve(__dirname, '../../bin/jemcashd')
 ...
 * @param {Object} options.config - The parsed bitcore-node-jemcash.json configuration file
...
function start(options) {
    ...
    fullConfig.path = path.resolve(options.path, './bitcore-node-jemcash.json');
...
function loadModule(req, service) {
    try {
        // first try in the built-in bitcore-node-jemcash services directory
