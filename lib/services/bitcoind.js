...
var BitcoinRPC = require('bitcoind-rpc-jemcash');
...
/**
 * Called by Node to determine the available API methods.
 * No CamelCase for Jemcash!

 Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    // commented out api methods are not supported by Jemcash
    ['getblock', this, this.getBlock, 1],
    // ['getrawblock', this, this.getRawBlock, 1],
    // ['getBlockHeader', this, this.getBlockHeader, 1],
    // ['getBlockOverview', this, this.getBlockOverview, 1],
...
Bitcoin.prototype._loadSpawnConfiguration = function (node) {
    ...
    var configPath = path.resolve(spawnOptions.datadir, './jemcash.conf');
...
Bitcoin.prototype._checkConfigIndexes = function (spawnConfig, node) {
    ...
    $.checkState(
        ...
        '"zmqpubhashblock" option is required to get event updates from jemcashd. ' +
    ...
    if (spawnConfig.reindex && spawnConfig.reindex === 1) {
        log.warn('Reindex option is currently enabled. This means that jemcashd is undergoing a reindex. ' +
            ...
            'of bitcore-node-jemcash services will start.');
        node._reindex = true;
    }
};
...
Bitcoin.prototype._getNetworkConfigPath = function () {
    var networkPath;
    if (this.node.network === bitcore.Networks.testnet) {
        networkPath = 'testnet3/jemcash.conf';
        if (this.node.network.regtestEnabled) {
            networkPath = 'regtest/jemcash.conf';
        }
    }
    return networkPath;
};
        ...
        if (!self.node.stopping) {
            self.syncPercentage(function (err, percentage) {
                if (err) {
                    ...
                    log.info('Jemcash Height:', self.height, 'Percentage:', percentage.toFixed(2));
                }
            });
        }
    }
    ...
    if (node._reindex) {
        interval = setInterval(function () {
            node.client.getBlockchainInfo(function (err, response) {
                ...
                log.info('Jemcash Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function (callback) {
    var self = this;
    var spawnOptions = this.options.spawn;
    var pidPath = spawnOptions.datadir + '/jemcashd.pid';
    ...
    function stopProcess() {
        fs.readFile(pidPath, 'utf8', function (err, pid) {
            ...
            try {
                log.warn('Stopping existing spawned jemcash process with pid: ' + pid);
                self._process.kill(pid, 'SIGINT');
            } catch (err) {
                if (err && err.code === 'ESRCH') {
                    log.warn('Unclean jemcash process shutdown, process not found with pid: ' + pid);
                    return callback(null);
                } else if (err) {
                    return callback(err);
                }
...
Bitcoin.prototype._spawnChildProcess = function (callback) {
    ...
    self._stopSpawnedBitcoin(function (err) {
        ...
        log.info('Starting jemcash process');
        ...
        self.spawn.process.once('exit', function (code) {
            if (!self.node.stopping) {
                log.warn('Jemcash process unexpectedly exited with code:', code);
                log.warn('Restarting jemcash child process in ' + self.spawnRestartTime + 'ms');
                setTimeout(function () {
                    self._spawnChildProcess(function (err) {
                        if (err) {
                            return self.emit('error', err);
                        }
                        log.warn('Jemcash process restarted');
                    });
        ...
        async.retry({times: 60, interval: self.startRetryInterval}, function (done) {
            ...
            if (exitShutdown) {
                return callback(new Error('Stopping while trying to spawn jemcashd.'));
            }
...
Bitcoin.prototype._connectProcess = function (config, callback) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function (done) {
        ...
        if (exitShutdown) {
            return callback(new Error('Stopping while trying to connect to jemcashd.'));
        }
 ...
 * @param {Object} txid - A jemcash transaction id
 * @param {Function} callback
 */
    ...
    if (this.spawn && this.spawn.process) {
        ...
        this.spawn.process.once('exit', function (code) {
            if (!exited) {
                exited = true;
                if (code !== 0) {
                    var error = new Error('jemcashd spawned process exited with status code: ' + code);
                    error.code = code;
        ...
        setTimeout(function () {
            if (!exited) {
                exited = true;
                return callback(new Error('jemcashd process did not exit'));
            }
