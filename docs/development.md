...
To develop bitcore-node-jemcash:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-jemcash.git
...
We now will setup symlinks in `bitcore-node-jemcash` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-jemcash
ln -s ~/bitcoind-rpc-jemcash
```
...
cd devnode
mkdir node_modules
touch bitcore-node-jemcash.json
touch package.json
```

Edit `bitcore-node-jemcash.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-jemcash/bin/bitcore-node-jemcash start
```